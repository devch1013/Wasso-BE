...
class GenerationViewTest(APITestCase):
    ...
    def setUp(self, mock_storage):
        ...
        self.user = User.objects.create_user(
            username="btestuser", identifier="testuser"
        )
        self.user1 = User.objects.create_user(
            username="atestuser", identifier="testuser1"
        )
        self.user2 = User.objects.create_user(
            username="dtestuser", identifier="testuser2"
        )
        self.user3 = User.objects.create_user(
            username="ctestuser", identifier="testuser3"
        )
        ...
        self.member1, self.generation_mapping1 = ApplyService.join_generation(
            self.user1, self.generation
        )
        self.member2, self.generation_mapping2 = ApplyService.join_generation(
            self.user2, self.generation
        )
        self.member3, self.generation_mapping3 = ApplyService.join_generation(
            self.user3, self.generation
        )
    ...
    def test_generation_members(self):
        ...
        response = self.client.get(
            reverse("generations-members", args=[self.generation.id])
        )
    ...
    def test_generation_events(self):
        """기수 이벤트 조회 테스트"""
        response = self.client.get(
            reverse("generations-events", args=[self.generation.id])
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_get_generation_stats(self):
        ...
        self.assertEqual(
            stats.filter(attendance__status=AttendanceStatus.PRESENT).count(), 1
        )
        self.assertEqual(
            stats.filter(attendance__status=AttendanceStatus.LATE).count(), 1
        )
        self.assertEqual(
            stats.filter(attendance__status=AttendanceStatus.ABSENT).count(), 1
        )
