...
class GoogleSheetTest(TestCase):
    def setUp(self):
        ...
        self.generation = Generation.objects.create(
            club=self.club, name="1기", start_date=date(2024, 1, 1)
        )
        ...
        self.mapping1 = GenMember.objects.create(
            member=self.member1, generation=self.generation
        )
        self.mapping2 = GenMember.objects.create(
            member=self.member2, generation=self.generation
        )
    ...
    def test_create_attendance_sheet(self, mock_build, mock_credentials):
        ...
        # 검증
        self.assertEqual(
            result, "https://docs.google.com/spreadsheets/d/test_spreadsheet_id"
        )
