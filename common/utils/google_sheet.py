...
def create_attendance_sheet(generation: Generation) -> str:
    ...
    for i in range(len(events)):
        column_data = [row[i + 1] for row in attendance_data]  # i+1은 이름 컬럼을 건너뛰기 위함
        ...
        },
        {
            'addConditionalFormatRule': {
                'rule': {
                    ...
                    'booleanRule': {
                        'condition': {'type': 'TEXT_EQ', 'values': [{'userEnteredValue': '출석'}]},
                        'format': {'backgroundColor': {'red': 0.85, 'green': 0.92, 'blue': 0.85}}
                    }
                }
            }
        },
        {
            'addConditionalFormatRule': {
                'rule': {
                    'ranges': [{'sheetId': 0, 'startRowIndex': 1}],
                    'booleanRule': {
                        'condition': {'type': 'TEXT_EQ', 'values': [{'userEnteredValue': '지각'}]},
        ...
        },
        {
            'addConditionalFormatRule': {
                'rule': {
                    'ranges': [{'sheetId': 0, 'startRowIndex': 1}],
                    'booleanRule': {
                        'condition': {'type': 'TEXT_EQ', 'values': [{'userEnteredValue': '결석'}]},
