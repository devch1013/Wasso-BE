...
class EventQRCheckView(ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            ...
            logger.warning(
                f"Event not found - event_id: {event_id}, user: {request.user}"
            )
            return Response(
        ...
        if event.qr_code != serializer.validated_data["qr_code"]:
            logger.warning(
                f"Invalid QR code - event_id: {event_id}, user: {request.user}"
            )
            return Response(
        ...
        if distance > 5:
            logger.warning(
                f"User too far from event location - event_id: {event_id}, user: {request.user}, distance: {distance}m"
            )
            return Response(
        ...
        if not (
            event.attendance_start_datetime <= now <= event.attendance_end_datetime
        ):
            logger.warning(
                f"Attendance time expired - event_id: {event_id}, user: {request.user}, current_time: {now}"
            )
            return Response(
        ...
        if existing_attendance:
            logger.warning(
                f"Duplicate attendance - event_id: {event_id}, user: {request.user}"
            )
            return Response(
        ...
        is_late = (
            now - event.attendance_start_datetime
        ).total_seconds() / 60 > event.late_tolerance_minutes

        Attendance.objects.create(
            user=request.user, event=event, is_present=True, is_late=is_late
        )

        return Response(
            {"status": "success", "code": 0, "message": "출석 완료", "is_late": is_late}
        )
