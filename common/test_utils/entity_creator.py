...
class EntityCreator:
    ...
    def create_club(self, club_name=None, user_name=None):
        """클럽과 기본 Role을 생성"""
        self.users.append(User.objects.create_user(username=user_name, identifier=user_name))
    ...
    def create_members(self, user_count=1):
        ...
        for user in users:
            member, generation_mapping = ApplyService.join_generation(user, self.generation)
            self.members.append(member)
    ...
    def create_attendances(self, status_list=None):
        ...
        status_list = status_list or [AttendanceStatus.PRESENT] * len(self.generation_mappings)
